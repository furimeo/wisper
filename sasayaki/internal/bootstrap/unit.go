...
# Binding :80 and :443 is the only privileged thing the daemon does itself. Everything
# that needs more - creating containers, mounting volumes, remapping users - is done by
# the Docker engine, which is its own service with its own privileges.
#
# If a future package needs the daemon to do one of those directly, add the capability
# here and say why in the commit: XFS project quotas need CAP_SYS_ADMIN, and changing the
# owner of a file the daemon did not create needs CAP_CHOWN.
